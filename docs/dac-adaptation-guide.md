...
iBasso UAC 1.8.6 的读取线程按 HID IN 端点的 `maxPacketSize` 保留完整报文，不能先裁剪成 16 字节。当前设备的 32 字节报文中，主动音量通知以字节 `[4]=0xFE`、`[5]=0x01` 标识，左右声道值位于 `[8]`、`[9]`；命令响应的命令号位于 `[6]`，负载长度位于 `[7]`，负载从 `[8]` 开始。报文长度、头偏移和字段含义都必须从对应产品的厂商实现或真机日志取证，不能根据 VID 或同系列型号推断。已经迟到但结构合法的命令响应应作为过期响应丢弃，不能误报为未知音量通知；例如 `...0000000178...` 表示命令 `0` 的单字节响应 `0x78`，不是 DAC 旋钮事件。
...
8. **硬件音量（若启用）**：从低响度开始，左右声道同步；0/25/50/100% 写入与读回一致；保留 HID IN 的完整 `maxPacketSize` 报文，并分别记录命令响应、迟到响应和外置旋钮通知的原始十六进制；切换数字/原始模式能恢复 0 dB；DoP/Native 无码流修改且 DAC 模拟响度确实变化。
